...
router.get('/queue/:id', middleware, async (req: Request, res: Response) => {
  const { id } = req.params
...
router.get('/queue/cancel/:id', middleware, async (req: Request, res: Response) => {
  const { id } = req.params
...
router.post('/report/:id', middleware, async (req: Request, res: Response) => {
  const { id } = req.params
  ...
  try {
    ...
    return res.status(500).json({
      error: `An error occured while processing league result for player: ${id}.`,
    })
  }
})
